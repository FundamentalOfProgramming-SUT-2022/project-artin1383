...
void compareText(char filename[]  , char filename2[]){
  FILE *f1 = fopen(filename, "r");
    ...
    while (1)
    {
        if(currentLine1[index-1]!= '\n'&& c1 != EOF) currentLine1[index] = c1;
        if(currentLine2[index-1]!= '\n' && c2 != EOF)currentLine2[index] = c2;
        index++;
      if((c1 == '\n' || c1 == EOF) && (c2 == '\n' || c2 == EOF) ) {
...
if(line[startIndex] == '\"'){
    ...
    if(line[startIndex] == '/') startIndex+=1;
    int baseIndex = startIndex;
    while(line[startIndex] != ' '&& startIndex != strlen(line) -1 ){
    fullpath[startIndex-baseIndex] = line[startIndex];
    startIndex++;
    }
    }
...
char* makestr(char* line , int startIndex){
  char* str  = malloc(200);
...
if(line[startIndex] == '\"'){
     startIndex++;
      int baseIndex = startIndex;
      while(1){
	if(line[startIndex] == '\"' && line[startIndex-1] != '\\') break;
        str[startIndex-baseIndex] = line[startIndex];
        startIndex++;
      }
    }
else {
...
void checkText(char filename[],char pattern[] , int option , int isArman){
    ...
    char* currentLine = malloc(1000);
    for (int i = 0; i < 1000; i++)currentLine[i] = 0;
    int index = 0;
    int haspattern = 0; 
    char c = getc(file);
    while (1)
  ...
  while (1){
    if(currentPosition == position && currentLine == targetline) {
	    for (int  i = 0; i < strlen(str); i++){
	     ...
	     if(str[i] == '\\' && str[i+1] == '\\' && str[i+2] == 'n') {
	           putc('/' , file2);
	      	   putc('n' , file2);
                    i+=2;
	      }
	      else  putc(str[i] , file2);
      };
    }
    putc(c , file2);
    currentPosition ++;
...
void tree(char *basePath, const int root , int depth , int isArman)
{
    ...
    while ((dp = readdir(dir)) != NULL)
    {
        if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0)
        {
            ...
            if(dp -> d_type ==DT_DIR) isArman ==1 ? fputc(':' , exportfirstcommand) :printf(":");
...
void handleCreatefile(char line[]){
    ...
    //if(access(path, F_OK==0)) printf("File is exist");
    if(file_exists(fullpath) == 1)  printf("File is exist");
    else {
...
void handleInsert(char line[]){
    ...
    str = makestr(line , 6);
    if(file_exists(fullpath) == 0)  printf("File Not Found");
    else insertText(fullpath , str , atoi(position) , atoi(targetline));
}
...
void handleRemove(char line[]){
    ...
    for (int i = 0; i < 200; i++) fullpath[i]=0;
        ...
        fullpath = makefullpath(line , i+7);
	 if(file_exists(fullpath) == 0)  printf("File Not Found");
	 else removeText(fullpath , atoi(position) , atoi(targetline) , atoi(size) , line[j+2]);
}
void handleCopy(char line[]){
    ...
    for (int k = startIndex+5; k < strlen(line); k++)
        ...
        fullpath = makefullpath(line , i+7);
        if(file_exists(fullpath) == 0)  printf("File Not Found");
	else copyText(fullpath , atoi(position) , atoi(targetline) , atoi(size) , line[startIndex+2]);
...
void handlePaste(char line[]){
    ...
    //char* str = (char *)hData;
    //CloseClipboard();
    FILE* file = fopen(".clipboard.txt"  , "r");
    char  str [1000];
...
void handleGrep(char line[] , int isArman){
  ...
  while (1)
  {
    ...
    startIndex++;
    baseIndex = startIndex;
    while(files[startIndex] != ' '&& startIndex != strlen(files)-1){
    filename[startIndex-baseIndex] = files[startIndex];
    startIndex++;
    }
    }
...
void handleCompare(char line[]){
    ...
    fullpath2 = makefullpath(line , start);
     if(file_exists(fullpath1) == 0)  printf("File 1 Not Found");
      if(file_exists(fullpath2) == 0)  printf("File 2  Not Found");
      else compareText(fullpath1 , fullpath2);
}
...
void handleInsertArman(char line[]){
    ...
    for (int k = 0; k < strlen(line); k++) position[k] = line[k+1];
     if(file_exists(fullpath) == 0)  printf("File Not Found");
     else insertArmanText(fullpath, atoi(position) , atoi(targetline));
}
...
void handleFind(char line [] , int isArman){
    ...
    fullpath = makefullpath(line , 7);
    if(file_exists(fullpath) == 0)  printf("File Not Found");
    if(isall ==1 && at!= 1) printf("you should not use these two command");

    else findText(fullpath , str , 0,iscount,at,isbyword,isall);
}
void handleReplace(char line [] , int isArman){
    ...
    fullpath = makefullpath(line , 7);
     if(file_exists(fullpath) == 0)  printf("File Not Found");
        if(isall ==1 && at!= 1) printf("you should not use these two command");
     else replaceText(fullpath , str1 , str2 ,0,at,isall);
}
/*int main (){
    char line[1000];
    char copyline[1000];
